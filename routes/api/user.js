...
router.post(
  ...
  [
    ...
    check("password", "Password shoud be more than 5 character").isLength({
      min: 5,
    }),
  ...
  async (req, res) => {
    ...
    const { name, phone, email, password, address, state, city, pincode } =
      req.body;
    try {
...
router.put(
  ...
  async (req, res) => {
    ...
    const { name, phone, email, address, state, city, pincode } = req.body;
    try {
